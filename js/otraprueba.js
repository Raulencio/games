...
var precioAtq=100;var precioCrit=1000;
function aumentarDMG(x){
switch(x){
case 0:
    var abc=confirm("incrementar "+(100*nivel)+" de ataque temporalmente por "+precioAtq+" ?");
    if(abc){
    if(oro>=precioAtq){oro-=precioAtq;precioAtq*=2;
    datosPj.ataque+=100*nivel;
    }}
break;
case 1:
    if(datosPj.probCrit<100){
var def=confirm("incrementar 5% la probabilidad de critico por "+precioCrit+" ?");
if(def){
    if(oro>=precioCrit){
        oro-=precioCrit;precioCrit=Math.ceil(precioCrit*2.5);
        datosPj.probCrit+=5;
    }
}
}else{
    alert("probabilidad critica maxima alcanzada");
}
break;
}
}